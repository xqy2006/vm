...
function webGetAsyncImpl(mp, path, fileRef, method, p)
{
	...
	dl.onload = function(dlData) {
		...
		else if(dlData.response)
		{
			...
			fileRef.fileLength = ci.fileLength = dlData.response.byteLength;
		...
		else if(dlData.fileLength >= 0)
		{
			...
			inodeId = ci.inodeId;
			permType = ci.permType = CheerpJFileData.S_IFREG | 0444;
			fileRef.fileLength = ci.fileLength = dlData.fileLength;
		}
...
function idbRemoveDEntry(mp, path)
{
...
function cheerpjOpenAsync(fds, path_, mode, p)
{
	...
	}else if(mode != "r" && !fileData.parent.hasReadWrite){
